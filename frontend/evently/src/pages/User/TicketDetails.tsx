...
import { getTicketDetails } from '../../Redux/slices/auth/authTicketSlice';
import TicketDetailsShimmer from '../../components/user/ShimmerUI/TicketDetailsShimmer';

const TicketDetails = () => {
    const { id } = useParams()
    const navigate = useNavigate()
    const dispatch = useDispatch<AppDispatch>()
    const { currentTicket, loading, } = useSelector((state: RootState) => state.authTickets);
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="px-3 py-1 rounded-full text-sm font-medium bg-white/20 text-white backdrop-blur-sm">
                                            {currentTicket?.event?.category?.name}
                                        </span>
                                    </div>
                                </div>
                            </div>
                ...
                {currentTicket?.event?.eventType === "offline" && (
                    <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                        <p className="text-sm text-blue-900">
                            <strong>Important:</strong> Please bring a valid ID and this ticket (digital or printed) to the event.
